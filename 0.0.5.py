...
"""финальная версия для оценки Крит; т.к. токен диска не разглашается, если вы считаете что он необходим вам для оценки
сообщите мне - grammarline@yandex.ru (без него загрузчик не будет работать, однако при наличии нужных файлов - игра запустится)"""
pg.init()
SERVER = yadisk.YaDisk(token='ВАШ_ТОКЕН_ДИСКА')
clock = pg.time.Clock()
